...
def Get1LineResult(point):
  return str(str(point[0][0]) + ","  + str(point[0][1])  + "," + str(point[1][0]) + ","  + str(point[1][1])  + "," + str(point[2][0]) + ","  + str(point[2][1])  + "," + str(point[3][0]) + ","  + str(point[3][1]) + "\n")  
...
class TextVisualizer(Visualizer):
    ...
    def draw_instance_predictions(self, predictions,threshold=0.5, path = ''):
        ...
        with open(path, 'w') as f:          
          for bezier, rec, score in zip(beziers, recs, scores):
            ...
            polygon = Get1LineResult(polygon)
            prob = score
            if prob > threshold:
                f.write(str(polygon).rstrip() + '\t' + str(prob) + '\n')
            results.append([polygon, prob])
        f.close()
        return results
