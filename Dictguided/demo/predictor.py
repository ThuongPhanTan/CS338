...
import numpy as np
import torch
from adet.utils.visualizer_v1 import TextVisualizer
...
class VisualizationDemo(object):
    ...
    def run_on_image(self, image, path, path_out=None,threshold=0.5):
        """
        Args:
        ...
        if "panoptic_seg" in predictions:
            ...
            vis_output = visualizer.draw_panoptic_seg_predictions(panoptic_seg.to(self.cpu_device), segments_info, path)
            ...
            if "instances" in predictions:
                ...
                #visualizer.save_output(predictions=instances, path_out = path_out)
                if path_out != None:
                    vis_output = visualizer.draw_instance_predictions(predictions=instances, path = path_out)
        return predictions
        ...
        def process_predictions(frame, predictions):
            ...
            if "panoptic_seg" in predictions:
                ...
                vis_frame = video_visualizer.draw_panoptic_seg_predictions(
                    frame, panoptic_seg.to(self.cpu_device), segments_info, path
                )
